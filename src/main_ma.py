        ...
        for run in range(num_runs):
            ...
            for agent_config in agent_configs:
                ...
                if "Competitor-Truthful" not in agent_config['name']:
                    total_item_features[run].append(temp)
...
def instantiate_agents(rng, agent_configs, num_auctions, total_item_features_run, obs_context_dim, update_interval, context_dist, random_bidding):
    ...
    agents = [
        Agent(rng=rng,
              ...
              allocator=eval(f"{agent_config['allocator']['type']}(rng=rng, total_item_features=total_item_features_run{parse_kwargs(agent_config['allocator']['kwargs'])})"),
...
if __name__ == '__main__':
    ...
    if args.num_auctions == 1:
        ...
        with open('config/training_mb.json') as f:
            training_config = json.load(f)        
    ...
    auction2run2agents2items, auction2run2agents2item_values, run2bilinear_map, total_item_features = draw_features(num_auctions, rng, num_runs, context_dim, feature_dim, agent_configs)

    # Repeated runs
    for run in range(num_runs):
        multi_auctions = []
        total_item_features_run = total_item_features[run]
        agents = instantiate_agents(rng, agent_configs, num_auctions, total_item_features[run], obs_context_dim, update_interval, context_dist, random_bidding)
